...
int main(int argc, char *argv[])
{
	...
	if (scene->HasMeshes())
	{
		...
		Curve c = Curve(glm::vec4(1.0f, 0.0f, 0.0f, 1.0f),
		...
		std::vector<glm::vec3> cList = c.getControlPolygon();
		std::vector<int> cIndeces = c.getControlPolygonAdjList();

		GLFWwindow *window;
		glfwInit();

		window = glfwCreateWindow(width, height, "Vjezba 8a -- krivulja Beziera", nullptr, nullptr);
		...
		glBufferData(GL_ARRAY_BUFFER, cList.size() * sizeof(glm::vec3), &cList[0], GL_STATIC_DRAW);

		glEnableVertexAttribArray(0);
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
		
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO[1]);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, cIndeces.size() * sizeof(int), &cIndeces[0], GL_STATIC_DRAW);

		Shader *shader2 = loadShader(argv[0], "shader2");
		Shader *shader = loadShader(argv[0], "shader");

		GLint uniformProj2 = glGetUniformLocation(shader2->ID, "matProjection");
		GLint uniformView2 = glGetUniformLocation(shader2->ID, "matView");
		GLint uniformModel2 = glGetUniformLocation(shader2->ID, "matModel");
		GLint uniformProj = glGetUniformLocation(shader->ID, "matProjection");
		GLint uniformView = glGetUniformLocation(shader->ID, "matView");

		GLint eyeView = glGetUniformLocation(shader2->ID, "eyeView");
		GLint materialPropsUniform = glGetUniformLocation(shader2->ID, "materialProps");
		...
		while (glfwWindowShouldClose(window) == false)
		{
			...
			glUniformMatrix4fv(uniformProj2, 1, GL_FALSE, &projection[0][0]);
			glUniformMatrix4fv(uniformView2, 1, GL_FALSE, &view[0][0]);
			...
			{
				glUniformMatrix4fv(uniformModel2, 1, GL_FALSE, &(objects[i]->transform->modelMatrix[0][0]));
			...
			glBindVertexArray(VAO[1]);
			glBindBuffer(GL_ARRAY_BUFFER, VBO[1]);
			glDrawArrays(GL_LINE_STRIP, 0, cList.size());
