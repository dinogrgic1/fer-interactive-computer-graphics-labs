...
Light light = Light(glm::vec3(0.0f, 0.0f, 1.0f));
...
void cursor_position_callback(GLFWwindow *window, double xpos, double ypos)
{
    ...
    camera->position += glm::vec3(0.001f * deltaX, 0.0f, 0.0f);
    camera->position += glm::vec3(0.0f, 0.001f * deltaY, 0.0f);
    view = Transform::lookAtMatrix(camera->getPosition(), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
int main(int argc, char *argv[])
{
    ...
    if (scene->HasMeshes())
    {
        ...
        Shader *shader = loadShader(argv[0], "constShade");
        ...
        GLint eyeView = glGetUniformLocation(shader->ID, "eye");
        ...
        while (glfwWindowShouldClose(window) == false)
        {
            ...
            glUniform3fv(eyeView, 1, &camera->getPosition()[0]);
            glUniform3fv(lightPosUniform, 1, &(light.getPosition()[0]));
